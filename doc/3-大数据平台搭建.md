...
```bash
    ...
    # 2. 添加 datadir 定义时的值：
        datadir=/opt/db/mysql/data                                             # sed -i 's/^datadir=$/basedir=\/opt\/db\/mysql\/data/g' ${MYSQL_HOME}/support-files/mysql.server
    ...
    # 6. 修改变量 libexecdir 的值
        libexecdir=/opt/db/mysql/data                                          # sed -i 's/^datadir=$/basedir=\/opt\/db\/mysql\/data/g' ${MYSQL_HOME}/support-files/mysql.server
        
    # 7. 将脚本中的 usr/local 统一替换为 opt/db
...
```bash
    # 修改变量 basedir 和 bindir 的值：
        ...
        bindir=/usr/local/mysql/bin  ==> bindir=/opt/db/mysql/bin              # sed -i 's/usr\/local/opt\/db/g' ${MYSQL_HOME}/support-files/mysqld_multi.server
```

#### 2.1.8 修改 ${MYSQL_HOME}/support-files/mysqld-log-rotate

```bash
    # 修改脚本中文件路径
    sed -i 's/usr\/local/opt\/db/g' ${MYSQL_HOME}/support-files/mysqld-log-rotate
```
