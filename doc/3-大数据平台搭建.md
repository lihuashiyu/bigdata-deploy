...
```bash
    ...
    mkdir -p /etc/mysql/                                                       # 创建 Mysql 配置文件目录（ubuntu 需要）
    
    touch /etc/mysql/my.cnf                                                    # 创建 Mysql 配置文件，内容如 2.1.5
    cp /etc/mysql/my.cnf /opt/db/mysql/docs/my.cnf                             # 备份配置文件
    sudo chown issac:issac -R /opt/db/mysql/                                   # 将安装目录 mysql 授权给 当前用户
    chmod -R 771 /opt/db/mysql/data/                                           # 修改 数据存储目录 权限
    chmod -R 771 /opt/db/mysql/bin-log/                                        # 修改 bin-log 数据存储目录 权限
    chmod -R 777 /opt/db/mysql/tmp/                                            # 修改 临时文件目录 权限
...
```bash
    # 1. 添加 basedir 定义时的值：
        basedir=/opt/db/mysql                                                  # sed -i 's/^basedir=$/basedir=\/opt\/db\/mysql/g' ${MYSQL_HOME}/support-files/mysql.server 
        
    # 2. 添加 datadir 定义时的值：
        datadir=/opt/db/mysql/data                                             # sed -i 's/^datadir=$/datadir=\/opt\/db\/mysql\/data/g' ${MYSQL_HOME}/support-files/mysql.server
        
    # 3. 添加 mysqld_pid_file_path 定义时的值：
        mysqld_pid_file_path=/opt/db/mysql/tmp/mysqld.pid                      # sed -i 's/^mysqld_pid_file_path=$/mysqld_pid_file_path=\/opt\/db\/mysql\/tmp\/mysqld.pid/g' ${MYSQL_HOME}/support-files/mysql.server
        
    # 4. 修改变量 sbin 的值
        sbindir="$basedir/sbin"        ==> sbindir="$basedir/bin"              # sed -i 's/sbindir="\$basedir\/sbin"/sbindir="\$basedir\/bin"/g' ${MYSQL_HOME}/support-files/mysql.server
    
    # 5. 修改变量 libexecdir 的值
        libexecdir="$basedir/libexec"  ==> libexecdir="$basedir/bin"           # sed -i 's/libexecdir="\$basedir\/libexec"/libexecdir="\$basedir\/bin"/g' ${MYSQL_HOME}/support-files/mysql.server
        
    # 6. 将脚本中的 usr/local 统一替换为 opt/db
    sed -i 's/usr\/local/opt\/db/g' ${MYSQL_HOME}/support-files/mysql.server
```

#### 2.1.7 修改 ${MYSQL_HOME}/support-files/mysql_multi.server

```bash
    # 修改变量 basedir 和 bindir 的值：
        basedir=/usr/local/mysql     ==> basedir=/opt/db/mysql
        bindir=/usr/local/mysql/bin  ==> bindir=/opt/db/mysql/bin              # sed -i 's/usr\/local/opt\/db/g' ${MYSQL_HOME}/support-files/mysql_multi.server
```

#### 2.1.8 修改 ${MYSQL_HOME}/support-files/mysql-log-rotate
...
#### 2.1.9 编写 ${MYSQL_HOME}/bin/mysql.sh 启停脚本
...
```xml
    <configuration>
        ...
        <property>
            ...
            <value>/opt/apache/hadoop/data/journalnode</value>
        </property>
        ...
        <!-- 两次 CheckPoint 之间的时间间隔周期，21600s -->
        <property>
            <name>dfs.namenode.checkpoint.period</name>
            <value>21600</value>
        </property>
        <!-- DataNode 的心跳时间，timeout = 2 * heartbeat.recheck.interval + 10 * dfs.heartbeat.interval -->
        <property>
            <name>dfs.heartbeat.interval</name>
            <value>3</value>
        </property>
        <!-- HDFS 在数据均衡时移动数据的速度：8388608（4MB/s） -->
        <property>
...
```xml
    <configuration>
        ...
        <!-- 存放已经运行完的 Hadoop 作业记录地址 -->
        <property>
            <name>mapreduce.jobhistory.done-dir</name>
            <value>/hadoop/history/finish</value>
        </property>
...
```xml
    <configuration>
        ...
        <!-- 定义 client 连接到 NodeManager 的最大超时时间，60s -->
        <property>
            <name>yarn.client.nodemanager-connect.max-wait-ms</name>
            <value>60000</value>
        </property>        
        <!-- 定义 client 每次尝试连接到 NodeManager 之间的时间间隔，10s -->
        <property>
        ...
        -->
        <!-- ResourceManager 逻辑 id 列表，以逗号分割 -->
        <!--
        <property>
...
#### 4.7.2 集群同步脚本 /usr/bin/xync.sh 

```bash
    sudo vim      /usr/bin/xync.sh                                             # 创建同步脚本，详见：/bigdata-deploy/script/system/xync.sh
    sudo chmod +x /usr/bin/xync.sh                                             # 添加可执行权限
```

#### 4.7.3 查看集群命令 /usr/bin/xcall.sh

```bash
    sudo vim      /usr/bin/xcall.sh                                            # 创建同步脚本，详见：/bigdata-deploy/script/system/xcall.sh
    sudo chmod +x /usr/bin/xcall.sh                                            # 添加可执行权限
```
...
```bash
    cd ~/coding/java/src  || exit                                              # 进入 spark 源码目录
    touch spark-3.2.3-hadoop-3.2.4.patch                                       # 创建 issac-spark 补丁，内容详见：/bigdata-deploy/deploy/patch/spark-3.2.3-hadoop-3.2.4.patch
    
    git apply --check ./spark-3.2.3-hadoop-3.2.4.patch                         # 检查 patch 是否可用
    git apply ./spark-3.2.3-hadoop-3.2.4.patch                                 # 应用补丁，不包含 commit 内容
    git am ./spark-3.2.3-hadoop-3.2.4.patch                                    # 应用补丁，包含 commit 内容
...
```bash
    touch issac-hive.patch                                                     # 创建补丁，补丁内容详见：/bigdata-deploy/deploy/patch/hive-3.1.3-spark-3.2.3.patch
    
    git apply --check ./hive-3.1.3-spark-3.2.3.patc                            # 检查 patch 是否可用
    git apply ./hive-3.1.3-spark-3.2.3.patch                                   # 应用补丁，不包含 commit 内容
    git am ./spark-3.2.3-hadoop-3.2.4.patch                                    # 应用补丁，包含 commit 内容
...
```bash
    ...
    rm ${HBASE_HOME}/lib/guava-11.0.2.jar                                      # 删除 hbase 旧版本的 guava
