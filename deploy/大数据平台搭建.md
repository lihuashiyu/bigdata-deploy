...
```bash
    # 详见 batch 文件夹下：/bigdata-deploy/shell/db/mysql.sh
```
...
#### 3.3.5 ${GRADLE_HOME}/gradle.properties
...
```bash
    ...
    ssh-copy-id issac@slaver*                                                  # 在 master 节点，将公钥复制到其它机器
    
    scp ~/.ssh/id_rsa.pub issac@master:~/.ssh/id_rsa.pub.slaver*               # 在 slaver节点上，将各自的 id_rsa.pub 发给 master 节点
...
#### 7.5.4 修改配置文件 ${KE_HOME}/conf/system-config.properties
...
#### 7.5.5 修改配置文件 ${KE_HOME}/conf/works
...
```bash
    ...
    touch ${KAFKA_HOME}/bin/kafka.sh                                           # 脚本内容，内容详见：/bigdata-deploy/shell/kafka.sh
    chmod +x ${KAFKA_HOME}/bin/kafka.sh                                        # 添加可执行权限
    touch ${KAFKA_HOME}/shell/kafka-cluster.sh                                 # 脚本内容，内容详见：/bigdata-deploy/other/kafka-cluster.sh
...
```mysql
    ...
    upsert into student (id, name, age, gender) values(1002, '李四', 36, 1);   -- 修改 
    upsert into student (id, name, age, gender) values(1001, '王五', 27, 1);   -- 添加 
