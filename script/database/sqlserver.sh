...
# =========================================================================================
#    FileName      ：  sqlserver.sh
#    CreateTime    ：  2024-12-06 14:34:03
#    Author        ：  lihua shiyu
#    Email         ：  lihuashiyu@github.com
#    Description   ：  sqlserver.sh 被用于 ==> sqlserver 的启停和状态检查脚本
# =========================================================================================
    
    
SQL_SERVER_HOME=$(cd "$(dirname "$(readlink -e "$0")")/../" || exit; pwd)      # 程序路径
ALIAS_NAME=SqlServer                                                           # 程序别名
MYSQL_SAFE=mysqld_safe                                                         # 用于启动 mysqld 服务，把信息记录到 error
                        
SERVICE_PORT=3306                                                              # 服务监控端口
MYSQLD_PORT=33060                                                              # SqlServer 协议端口

MYSQL_LIST=(${sqlserver_list})                                           # SqlServer 的安装主机
LOG_FILE="sqlserver-$(date +%F).log"                                           # 启停操作日志文件
RUNNING=1                                                                      # 运行状态
STOP=0                                                                         # 停止状态
...
function service_status()
{
    ...
    do
        ...
        mysqld_pid=$(ssh "${USER}@${host_name}" "source ~/.bashrc; source /etc/profile; ${SQL_SERVER_HOME}/support-files/mysql.server status"  | awk -F '(' '{print $2}' | awk -F ')' '{print $1}')
...
function service_start()
{
    ...
    if [ "${status}" == "${RUNNING}" ]; then
        ...
        for host_name in "${MYSQL_LIST[@]}"
        do
            ssh "${USER}@${host_name}" "source ~/.bashrc; source /etc/profile; ${SQL_SERVER_HOME}/support-files/mysql.server start >> ${SQL_SERVER_HOME}/logs/${LOG_FILE} 2>&1"
        done
...
function service_stop()
{
    ...
    if [ "${status}" == "${STOP}" ]; then
        ...
        for host_name in "${MYSQL_LIST[@]}"
        do
            ssh "${USER}@${host_name}" "source ~/.bashrc; source /etc/profile; ${SQL_SERVER_HOME}/support-files/mysql.server stop >> ${SQL_SERVER_HOME}/logs/${LOG_FILE} 2>&1"
        done
...
function service_restart()
{
    ...
    if [ "${status}" == "${STOP}" ]; then
        ...
        for host_name in "${MYSQL_LIST[@]}"
        do
            ssh "${USER}@${host_name}" "source ~/.bashrc; source /etc/profile; ${SQL_SERVER_HOME}/support-files/mysql.server restart >> ${SQL_SERVER_HOME}/logs/${LOG_FILE} 2>&1"
        done
...
function service_reload()
{
    ...
    if [ "${status}" == "${STOP}" ]; then
        ...
        for host_name in "${MYSQL_LIST[@]}"
        do
            ssh "${USER}@${host_name}" "source ~/.bashrc; source /etc/profile; ${SQL_SERVER_HOME}/support-files/mysql.server reload >> ${SQL_SERVER_HOME}/logs/${LOG_FILE} 2>&1"
        done
