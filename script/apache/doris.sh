...
DORIS_DIR=$(cd -P "$(dirname "$(readlink -e "$0")")" || exit; pwd -P)          # Doris 安装目录
ALIAS_NAME="Doris"                                                             # FE 别名
ALIAS_FE_NAME="Doris FE"                                                       # FE 别名
ALIAS_BE_NAME="Doris BE"                                                       # FE 别名
ALIAS_BROKER_NAME="Doris Broker"                                               # FE 别名
    
FE_NAME=org.apache.doris.PaloFe
BE_NAME=lib/palo_be
BROKER_NAME=org.apache.doris.PaloFe
...
FE_LIST=(issac issac issac)                                                                # FE     部署节点
BE_LIST=(issac issac issac)                                                                # BE     部署节点
BROKER_LIST=(  )                                                            # Broker 部署节点
...
function node_service_status()
{
    ...
    do
        # 3. 程序 的 pid
        pid_count=$(ssh "${USER}@${host_name}" "source ~/.bashrc; source /etc/profile; ps -aux | grep -i '${USER}' | grep -i '$2' | grep -v grep | wc -l")
...
# 各个节点服务状态启动（$1：部署服务的节点列表、$1：服务名称、$1：服务别名）
function service_status()
{
    # 1. 定义变量
    local fe_status be_status broker_status
    ...
    # 4. 遍历 Broker 的所有的主机，查看进程
    broker_status=$(node_service_status "${BROKER_LIST[*]}" "${BROKER_NAME}" "${ALIAS_BROKER_NAME}")
    
    # 5. 判断是否所有的进程都正常
    if [[ "${fe_status}" == "${RUNNING}" ]] && [[ "${be_status}" == "${RUNNING}" ]] && [[ "${broker_status}" == "${RUNNING}" ]]; then 
         echo "${RUNNING}"
    elif [[ "${fe_status}" == "${STOP}" ]] && [[ "${be_status}" == "${STOP}" ]] && [[ "${broker_status}" == "${STOP}" ]]; then
        echo "${STOP}"
    else
        if [[ "${fe_status}" != "${RUNNING}" ]] || [[ "${fe_status}" != "${STOP}" ]]; then
            echo "${fe_status}"
        fi
        
        if [[ "${be_status}" != "${RUNNING}" ]] || [[ "${be_status}" != "${STOP}" ]]; then
            echo "${be_status}"
        fi
        
        if [[ "${broker_status}" != "${RUNNING}" ]] || [[ "${broker_status}" != "${STOP}" ]]; then
            echo "${broker_status}"
        fi        
    fi
}
...
function fe_start()
{
    ...
    if [ "${fe_status}" == "${RUNNING}" ]; then
        ...
        do
            ssh "${USER}@${host_name}" "source ~/.bashrc; source /etc/profile; ${DORIS_DIR}/fe/bin/start_fe.sh --daemon >> ${DORIS_DIR}/fe/log/${FE_LOG_FILE} 2>&1 "
        done
...
function be_start()
{
    ...
    if [ "${be_status}" == "${RUNNING}" ]; then
        ...
        for host_name in "${FE_LIST[@]}"
        do
            ssh "${USER}@${host_name}" "source ~/.bashrc; source /etc/profile; ${DORIS_DIR}/be/bin/start_be.sh --daemon >> ${DORIS_DIR}/be/log/${BE_LOG_FILE} 2>&1 "
        done
...
function broker_start()
{
    ...
    if [ "${broker_status}" == "${RUNNING}" ]; then
        ...
        for host_name in "${FE_LIST[@]}"
        do
            ssh "${USER}@${host_name}" "source ~/.bashrc; source /etc/profile; ${DORIS_DIR}/broker/bin/start_broker.sh --daemon >> ${DORIS_DIR}/broker/log/${BROKER_LOG_FILE} 2>&1 "
        done
...
function fe_stop()
{
    ...
    if [ "${fe_status}" == "${STOP}" ]; then
        ...
        for host_name in "${FE_LIST[@]}"
        do
            ssh "${USER}@${host_name}" "source ~/.bashrc; source /etc/profile; ${DORIS_DIR}/fe/bin/stop_fe.sh >> ${DORIS_DIR}/fe/log/${FE_LOG_FILE} 2>&1 "
        done
...
function be_stop()
{
    ...
    if [ "${be_status}" == "${STOP}" ]; then
        ...
        for host_name in "${FE_LIST[@]}"
        do
            ssh "${USER}@${host_name}" "source ~/.bashrc; source /etc/profile; ${DORIS_DIR}/be/bin/stop_be.sh >> ${DORIS_DIR}/be/log/${BE_LOG_FILE} 2>&1 "
        done
...
function service_stop()
{
    ...
    if [ "${status}" == "${STOP}" ]; then
        ...
        do
            ssh "${USER}@${host_name}" "source ~/.bashrc; source /etc/profile; ${DORIS_DIR}/broker/bin/stop_broker.sh >> ${DORIS_DIR}/broker/log/${BROKER_LOG_FILE} 2>&1 "
        done
